...
class Player(wavelink.Player):
    ...
    async def add_tracks(self, ctx, tracks):
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            await ctx.reply(f"Track ({tracks[0].title}) added.")

        else:
            if (track := await self.choose_track(ctx, tracks)) is not None:
                self.queue.add(track)

                await ctx.reply(f"Track ({track.title}) added.")
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def cog_check(self, ctx):
        if isinstance(ctx.channel, discord.DMChannel):
            await ctx.reply("Music commands can't be used in DMs.")
            return False
    ...
    @commands.command(
        name="connect", aliases=["join"], brief="Connect Palmtrwwpy to a Voice Channel"
    )
    async def connect_command(
        self, ctx, *, channel: typing.Optional[discord.VoiceChannel]
    ):
        """Connect Palmtrwwpy to a Voice Channel to be able to listen to music!"""
    ...
    @commands.command(
        name="disconnect", aliases=["leave"], brief="Make Palmtrwwpy leave a voice channel."
    )
    async def disconnect_command(self, ctx):
        """Disonnect Palmtrwwpy from a voice channel."""
        player = self.get_player(ctx)
    ...
    async def play_command(self, ctx, *, query: typing.Optional[str]):
        """Lets you be able to search a song, or put in a YouTube link so you can listen to music.\nIf Palmtrwwpy isn't already in  the voice channel, it will connect.\nIf Palmtrwwpy is already connected, it will queue up the song you requested."""
        player = self.get_player(ctx)
    ...
    async def stop_command(self, ctx):
        """Stops the music and clears the queue, Palmtrwwpy also leaves the Voice Channel."""
        player = self.get_player(ctx)

        player.queue.empty()

        await player.stop()
        await ctx.reply("Music stopped, queue cleared. ⏹")
    ...
    async def next_command(self, ctx):
        ...
        await player.stop()
        await ctx.reply("Skipping track. ▶")

    @next_command.error
    async def next_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.reply("Queue is empty, can't skip track.")

        if isinstance(exc, NoMoreTracks):
            await ctx.reply("No more tracks in the queue, can't skip track.")
    ...
    async def previous_command(self, ctx):
        ...
        await player.stop()
        await ctx.reply("Previous track is now playing. ◀")

    @previous_command.error
    async def previous_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.reply("Queue is empty, can't go to the previous track.")
